...
```hcl
provider "waldur" {
  # Endpoint and token are optional if environment variables are set
...
|----------|-------------|----------|---------|
| `endpoint` | The Waldur API endpoint URL | No | `WALDUR_API_URL` env var |
| `token` | API authentication token | No | `WALDUR_ACCESS_TOKEN` env var |
